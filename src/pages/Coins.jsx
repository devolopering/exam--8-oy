...
export default function Coins() {
  ...
  const customTheme = {
    ...
    pages: {
      ...
      previous: {
        base: "ml-0 rounded-full border-none bg-transparent px-3 py-3 mt-2 leading-tight text-[#87CEEB] enabled:hover:bg-transparent enabled:hover:hover:text-[#87CEEB] dark:border-gray-700 dark:bg-transparent dark:text-[#87CEEB] enabled:dark:hover:bg-gray-700 enabled:dark:hover:text-white",
        icon: "h-5 w-5"
      },
      next: {
        base: "rounded-full border-none bg-transparent px-3 py-3 mt-2 leading-tight text-[#87CEEB] enabled:hover:bg-transparent enabled:hover:hover:text-[#87CEEB] dark:border-gray-700 dark:bg-transparent dark:text-[#87CEEB] enabled:dark:hover:bg-gray-700 enabled:dark:hover:text-white",
        icon: "h-5 w-5"
      },
      selector: {
  ...
  return (
    <div className="bg-[#14161A] max-w-[1920px] mx-auto mt-[78px]">
      <HeroSection />
      {loading && <p className='mt-[78px] font-bold text-3xl text-white font-montserrat'>Loading...</p>}
      {error && <p className='mt-[78px] text-3xl font-medium text-red-500'>{error}</p>}
      {!loading && !error && (
                  ...
                  <Table.Row key={coin.id} className="dark:bg-transparent dark:border-gray-700">
                    ...
                    <Table.Cell className='text-white'>
                      {currency === 'USD' ? `$ ${coin.market_cap}` : 
                       ...
                       currency === 'RUB' ? `₽ ${coin.market_cap}` : coin.market_cap}
                    </Table.Cell>
                  </Table.Row>
                ))}
              </Table.Body>
